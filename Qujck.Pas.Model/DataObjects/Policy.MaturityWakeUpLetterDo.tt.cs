//------------------------------------------------------------------------------
// <auto-generated>
//    This code was generated from a template.
//
//    Manual changes to this file may cause unexpected behavior in your application.
//    Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------
using System;
using System.Collections.Generic;
using System.IO;
using System.Linq;
using Qujck.Pas.Core.Enumerations;

namespace Qujck.Pas.Model.Policy.DataObjects
{
    [Serializable]
    internal sealed partial class MaturityWakeUpLetterDo : AbstractModel
    {
        internal MaturityWakeUpLetterDo() 
        {
            this.statusValue = new MaturityWakeUpLetterState();
        }
        
        public bool effectiveDateHasValue { get; internal set; }
        private System.DateTime effectiveDateValue  { get; set; }
        public System.DateTime effectiveDate 
        {
            get
            {
                return this.effectiveDateValue;
            }
            internal set
            {
                this.effectiveDateHasValue = true;
                this.effectiveDateValue = value;
            }
        }

        public bool statusHasValue { get; internal set; }
        private MaturityWakeUpLetterState statusValue  { get; set; }
        public MaturityWakeUpLetterState status 
        {
            get
            {
                return this.statusValue;
            }
            internal set
            {
                this.statusHasValue = true;
                this.statusValue = value;
            }
        }

        public bool Maturity_idHasValue { get; internal set; }
        private long Maturity_idValue  { get; set; }
        public long Maturity_id 
        {
            get
            {
                return this.Maturity_idValue;
            }
            internal set
            {
                this.Maturity_idHasValue = true;
                this.Maturity_idValue = value;
            }
        }

        public bool Contract_idHasValue { get; internal set; }
        private long Contract_idValue  { get; set; }
        public long Contract_id 
        {
            get
            {
                return this.Contract_idValue;
            }
            internal set
            {
                this.Contract_idHasValue = true;
                this.Contract_idValue = value;
            }
        }
    }
}
